...
struct perf_event_attr *create_event_attr(uint64_t perf_event_id)
{
...
void init_events_list(ctr_list_t **events_list, const uint64_t *events_id, const char **events_name, const int len, uint64_t sample_period) 
{
    *events_list = malloc(sizeof(ctr_list_t));
    ...
    for(idx_item = 0; idx_item < len; idx_item++)
    {
        struct perf_event_attr *attr;
        if(idx_item == 0)
            attr = create_sample_event_attr(events_id[idx_item], sample_period);
        else
            attr = create_event_attr(events_id[idx_item]);
        ctr_t *ctr = create_ctr(attr);
        ctr->event_name = events_name[idx_item];
        int ret;
